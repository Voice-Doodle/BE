...
public class VodleController {
    ...
    public ResponseEntity<? extends BaseResponseBody> createVodle(
            ...
            @Parameter(name = "vodleCreateReqDto", description = """
                    ...
                    3.음성에 대한 텍스트 정보 : String text
                                    
                    4.변조된 음성 스트리밍 URL : String streamingURL
                                
                    5.위도에 대한 정보 : Double longitude
                                        
                    6.경도에 대한 정보 : Double latitude
                    """)
    ...
    @Operation(summary = "목소리 샘플 리스트 조회", description = "\n\n사용자는 사용가능한 목소리 샘플 리스트 조회가 가능하다.")
    @GetMapping("/samples")
    @ApiResponse(responseCode = "200", description = "성공 \n\n 사용가능한 목소리 샘플 리스트 반환",
        content = {@Content(mediaType = "application/json",
            schema = @Schema(implementation = DataResponseBody.class),
            examples = {
                @ExampleObject(name = "목소리 샘플 조회 성공", description = "사용자는 사용가능한 목소리의 샘플들을 조회시 위와 같은 응답 데이터를 받는다.", value = """
                    {
                        "status": 200,
                        "message": "목소리 샘플들 조회 완료",
                        "code": "SUCCESS",
                        "data": [
                            {
                                "voiceType": "Ralo",
                    ...
                    """),
                @ExampleObject(name = "샘플 목소리 조회 실패", description = "사용자는 사용가능한 목소리의 샘플 조회 실패시 위와 같은 응답 데이터를 받는다.", value = """
                    {
                            "status": 400,
                            "message": "샘플 목소리가 존재하지 않음",
                            "code": "S-001"
                    }
                    """)
            })
    ...
    public ResponseEntity<? extends DataResponseBody> getVoiceSamples() {
        List<VoiceSampleResDto> samples = vodleService.getListVoiceSample();
        return ResponseEntity.status(HttpStatus.OK).body(DataResponseBody.of(200, "샘플 목소리 리스트 조회 완료", "SUCCESS", samples));
    }
