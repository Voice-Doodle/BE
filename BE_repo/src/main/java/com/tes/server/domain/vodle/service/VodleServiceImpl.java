...
public class VodleServiceImpl implements VodleService {
    ...
    public void createVodle(String userCode, VodleCreateReqDto vodleCreateReqDto, MultipartFile soundFile) {
        ...
        try {
            ...
            // STS , NONE 경우
            if(!vodleCreateReqDto.getRecordType().equals(RecordType.TTS)) {
                ...
                classification = classificationFuture.join();
                ...
                // S3에 저장될 원본 파일명 생성
                inputKey = vodleCreateReqDto.getStreamingURL()
            ...
            // TTS 경우
                ...
                log.info("Classification 작업 시작 : {}", "START");
                // Classification 작업을 비동기적으로 실행합니다.
                CompletableFuture<String> classificationFuture = CompletableFuture.supplyAsync(() -> {
                    return aiApiClient.classifyText(vodleCreateReqDto.getText());
                });
...
//                vodleTtsReqDto.setSelected_voice(voiceType);
    ...
    @Override
    public List<VoiceSampleResDto> getListVoiceSample() {
        try{
            ...
            return voiceSampleResDtos;
        }catch (Exception e){
            throw new Exceptions(ErrorCode.VOICE_NOT_EXIST_DB);
        }
    }
}